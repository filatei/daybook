...
router.post('/ai/expense-parse', requireAuth, needTenant('SECRETARY'), async (req, res) => {
  ...
  const sites = await qall('SELECT id, name, code FROM sites WHERE tenant_id=?', [req.ctx.tenant_id]);
  const cats = (await qall("SELECT DISTINCT category FROM expenses WHERE tenant_id=? AND category IS NOT NULL", [req.ctx.tenant_id])).map((r) => r.category);
  const allCats = [...new Set([...cats, 'FUEL', 'MAINTENANCE', 'UTILITIES', 'SUPPLIES', 'SALARY', 'TRANSPORT', 'OTHER'])];
  const system = `You convert a Nigerian water-business expense described in plain English into JSON for a form. Money is Naira.
Return ONLY a JSON object with keys:
  site_name (string, must match one of the SITES by name/code, else ""),
  vendor (string),
  category (UPPERCASE, pick the closest from CATEGORIES, else "OTHER"),
  kind ("IMPREST" if it's a daily petty-cash/imprest expense, else "NON_IMPREST"),
  items (array of { name, qty (number), price (number, unit price in Naira) }),
  description (short purpose string).
If a quantity or unit price is not stated, infer qty=1 and put the stated amount as price. Never invent items not mentioned.
SITES: ${JSON.stringify(sites.map((s) => ({ name: s.name, code: s.code })))}
CATEGORIES: ${JSON.stringify(allCats)}`;
  try {
    const reply = await callAI({ system, messages: [{ role: 'user', content: text }], maxTokens: 600 });
    const j = parseAiJson(reply);
    if (!j) return res.status(422).json({ error: "couldn't understand that — try naming the vendor, item and amount" });
    const site = sites.find((s) => [s.name, s.code].some((x) => String(x).toLowerCase() === String(j.site_name || '').toLowerCase()))
      || (siteBound(req.ctx) ? sites.find((s) => s.id === req.ctx.site_id) : null);
    const items = Array.isArray(j.items) ? j.items.map((it) => ({ name: String(it.name || '').trim(), qty: Number(it.qty) || 1, price: Number(it.price) || 0 })).filter((it) => it.name) : [];
    res.json({
      draft: {
        site_id: site ? site.id : null, site_name: site ? site.name : null,
        vendor: (j.vendor || '').trim() || null,
        category: String(j.category || 'OTHER').toUpperCase(),
        kind: j.kind === 'IMPREST' ? 'IMPREST' : 'NON_IMPREST',
        description: (j.description || '').trim() || null,
        items, total: items.reduce((a, it) => a + it.qty * it.price, 0),
      },
    });
  } catch (e) { res.status(e instanceof AIError ? e.httpStatus : 502).json({ error: e.userMessage || e.message, code: e.code }); }
});
...
router.post('/ai/cash-parse', requireAuth, needTenant('SECRETARY'), async (req, res) => {
  ...
  const sites = await qall('SELECT id, name, code FROM sites WHERE tenant_id=?', [req.ctx.tenant_id]);
  const banks = (await qall("SELECT label FROM bank_accounts WHERE tenant_id=? AND active=1", [req.ctx.tenant_id])).map((r) => r.label);
  const system = `You convert a described cash deposit (cash collected, paid into the bank or via a POS agent) into JSON. Money is Naira.
Return ONLY a JSON object: { amount (number), depositor (string, who paid it in), site_name (match a SITE name/code or ""), payee_account (match a BANK label or "" ), memo (string) }.
SITES: ${JSON.stringify(sites.map((s) => ({ name: s.name, code: s.code })))}
BANKS: ${JSON.stringify(banks)}`;
  try {
    const reply = await callAI({ system, messages: [{ role: 'user', content: text }], maxTokens: 400 });
    const j = parseAiJson(reply);
    if (!j) return res.status(422).json({ error: "couldn't understand that — try stating the amount and depositor" });
    const site = sites.find((s) => [s.name, s.code].some((x) => String(x).toLowerCase() === String(j.site_name || '').toLowerCase()))
      || (siteBound(req.ctx) ? sites.find((s) => s.id === req.ctx.site_id) : null);
    const payee = banks.find((b) => b.toLowerCase() === String(j.payee_account || '').toLowerCase()) || (j.payee_account || '').trim() || null;
    res.json({ draft: { amount: Number(j.amount) || 0, depositor: (j.depositor || '').trim() || null, site_id: site ? site.id : null, site_name: site ? site.name : null, payee_account: payee, memo: (j.memo || '').trim() || null } });
  } catch (e) { res.status(e instanceof AIError ? e.httpStatus : 502).json({ error: e.userMessage || e.message, code: e.code }); }
});
...
router.use('/cash',      require('./routes_cash'));
