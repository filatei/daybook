...
async function sendGeneratedReport({ tenant, date, report, incidents, to, attachments = [] }) {
  ...
  <div style="font-family:Segoe UI,Arial,sans-serif;max-width:640px;margin:auto;color:#1f2937">
    ...
    <div style="border:1px solid #e5e7eb;border-top:none;border-radius:0 0 10px 10px;padding:22px">
      ...
      ${(s.salesByProductIncentive && s.salesByProductIncentive.length) ? `<div style="font-weight:800;margin:6px 0">Incentive / free items <span style="font-weight:600;color:#6b7280;font-size:12px">(not part of the sales total)</span></div>
      <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:14px">
        <thead><tr><th style="text-align:left;padding:5px 10px;color:#6b7280">Product</th><th style="text-align:right;padding:5px 10px;color:#6b7280">Qty</th><th style="text-align:right;padding:5px 10px;color:#6b7280">Amount</th></tr></thead>
        <tbody>
          ${s.salesByProductIncentive.map((p) => `<tr><td style="padding:4px 10px;border-bottom:1px solid #f0f0f0">${esc(p.name)}</td><td style="padding:4px 10px;border-bottom:1px solid #f0f0f0;text-align:right">${(p.qty || 0).toLocaleString()}</td><td style="padding:4px 10px;border-bottom:1px solid #f0f0f0;text-align:right">₦${(Number(p.amount) || 0).toLocaleString()}</td></tr>`).join('')}
        </tbody>
      </table>` : ''}
